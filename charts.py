...
if decision == "GREENLIGHT":
    ...
    <div class="decision-card decision-greenlight">
        <h3 style="color: #2e7d32; margin-top: 0;">✅ GREENLIGHT</h3>
        <p style="color: #1b5e20; margin: 0.5rem 0 0 0;">This experiment shows statistically significant improvement. Roll out the change.</p>
    </div>
    """, unsafe_allow_html=True)
elif decision == "KEEP RUNNING":
    st.markdown("""
    <div class="decision-card decision-warning">
        <h3 style="color: #f57f17; margin-top: 0;">🟨 KEEP RUNNING</h3>
        <p style="color: #e65100; margin: 0.5rem 0 0 0;">Results are inconclusive. Continue the experiment to gather more data or increase sample size.</p>
    </div>
    """, unsafe_allow_html=True)
elif decision == "STOP / REVERT":
    st.markdown("""
    <div class="decision-card decision-danger">
        <h3 style="color: #c62828; margin-top: 0;">🔴 STOP / REVERT</h3>
        <p style="color: #b71c1c; margin: 0.5rem 0 0 0;">This experiment shows statistically significant degradation. Stop and revert the change.</p>
    </div>
    """, unsafe_allow_html=True)
else:
    st.markdown("""
    <div class="decision-card decision-warning">
        <h3 style="color: #f57f17; margin-top: 0;">ℹ️ LOW IMPACT</h3>
        <p style="color: #e65100; margin: 0.5rem 0 0 0;">Result is significant but effect is small. Evaluate business value vs. implementation cost.</p>
    </div>
    """, unsafe_allow_html=True)
...
with tab1:
    ...
    with comparison_col1:
        ...
        st.caption(f"n = {int(experiments_results_summary_df['control_size']):,} users")
    
    with comparison_col2:
        st.markdown(f"**vs.**")
        st.markdown(f"<h3 style='margin-top: 2rem;'>→</h3>", unsafe_allow_html=True)
    
    with comparison_col3:
        st.markdown(f"**Treatment Group**")
        st.markdown(f"<h2 style='color: #ff7f0e; margin: 0;'>{treatment_rate:.2f}%</h2>", unsafe_allow_html=True)
        st.caption(f"n = {int(experiments_results_summary_df['treatment_size']):,} users")
    ...
    with analysis_col1:
        st.markdown("""
        <div style="background: linear-gradient(135deg, #e3f2fd 0%, #f3e5f5 100%); border-left: 5px solid #1976d2; border-radius: 8px; padding: 1.5rem; margin: 1rem 0;">
            <h4 style="color: #1565c0; margin-top: 0; margin-bottom: 1rem;">📍 What Happened</h4>
            <p style="color: #424242; line-height: 1.6; margin: 0;">""" + summary + """</p>
        </div>
        """, unsafe_allow_html=True)
    
    with analysis_col2:
        st.markdown("""
        <div style="background: linear-gradient(135deg, #f3e5f5 0%, #e0f2f1 100%); border-left: 5px solid #7b1fa2; border-radius: 8px; padding: 1.5rem; margin: 1rem 0;">
            <h4 style="color: #6a1b9a; margin-top: 0; margin-bottom: 1rem;">🎯 Next Steps</h4>
            <p style="color: #424242; line-height: 1.6; margin: 0;">""" + recommendation + """</p>
        </div>
        """, unsafe_allow_html=True)
...
with tab3:
    ...
    conversion_rates_fig.update_layout(
        ...
        plot_bgcolor='#ffffff',
        paper_bgcolor='white',
        font=dict(size=12, family="sans-serif", color='#1f1f1f'),
        title=dict(
            text=f"{experiment} — Conversion Rate Trends ({time_option})",
            font=dict(size=16, color='#0d47a1', family="sans-serif"),
            x=0.5,
            xanchor='center'
        ),
        ...
        xaxis=dict(
            ...
            linewidth=2,
            linecolor='#333',
            tickfont=dict(size=11, color='#1f1f1f')
        ),
        yaxis=dict(
            showgrid=True,
            gridwidth=1,
        ...
        legend=dict(
            ...
            bordercolor='#333',
            borderwidth=2,
            font=dict(size=12, color='#1f1f1f', family="sans-serif")
        )
    )
